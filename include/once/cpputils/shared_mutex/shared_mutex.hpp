...
{
public:
  SharedLock(SharedMutex& shared_mtx)
...
{
public:
  ExclusiveLock(SharedMutex& shared_mtx)
    : shared_mtx_{ shared_mtx }
    , locked_{ false }
  {
    lock();
  ...
  void lock()
  {
    if (!locked_) {
      std::unique_lock<std::mutex> lock{ shared_mtx_.mtx_ };
      shared_mtx_.cv_.wait(lock, [&] {
        return (0 == shared_mtx_.shared_counter_) &&
               !shared_mtx_.exclusively_locked_;
      });
