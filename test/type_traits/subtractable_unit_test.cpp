...
#include <once/cpputils/type_traits/subtractable.hpp>
...
SCENARIO("is_subtractable trait")
{
  GIVEN("an arithmetic type")
  {
    THEN("the trait value shall be true")
    {
      REQUIRE(is_subtractable<bool>::value);
      REQUIRE(is_subtractable<char>());
  ...
  GIVEN("an enum type")
  {
    ...
    {
      REQUIRE_FALSE(is_subtractable_v<MyEnum>);
    }
  }
  ...
  GIVEN("a pointer type")
  {
    THEN("the trait value shall be true")
    {
      struct Foo
      {};
      REQUIRE(is_subtractable_v<std::add_pointer_t<Foo>>);
    }
  }
  ...
  GIVEN("a type with a user-defined operator+")
  {
    THEN("the trait value shall be true")
    {
      ...
      REQUIRE(is_subtractable_v<Foo>);
      REQUIRE(is_subtractable_v<Bar>);
    }
  }
}