...
#include <once/cpputils/type_traits/divisible.hpp>
...
SCENARIO("is_divisible trait")
{
  GIVEN("an arithmetic type")
  {
    THEN("the trait value shall be true")
    {
      REQUIRE(is_divisible<bool>::value);
      REQUIRE(is_divisible<char>());
  ...
  GIVEN("an enum type")
  {
    ...
    {
      REQUIRE_FALSE(is_divisible_v<MyEnum>);
    }
  }
  ...
  GIVEN("a pointer type")
  {
    THEN("the trait value shall be false")
    {
      struct Foo
      {};
      REQUIRE_FALSE(is_divisible_v<std::add_pointer_t<Foo>>);
    }
  }
  ...
  GIVEN("a type with a user-defined operator+")
  {
    THEN("the trait value shall be true")
    {
      ...
      REQUIRE(is_divisible_v<Foo>);
      REQUIRE(is_divisible_v<Bar>);
    }
  }
}