...
#include <once/cpputils/type_traits/modulable.hpp>
...
SCENARIO("is_modulable trait")
{
  GIVEN("an arithmetic type")
  {
    THEN("the trait value shall be true")
    {
      REQUIRE(is_modulable<bool>::value);
      REQUIRE(is_modulable<char>());
      REQUIRE(is_modulable_v<int>);
    }
  }
  ...
  GIVEN("an enum type")
  {
    ...
    {
      REQUIRE_FALSE(is_modulable_v<MyEnum>);
    }
  }
  ...
  GIVEN("a pointer type")
  {
    THEN("the trait value shall be false")
    {
      struct Foo
      {};
      REQUIRE_FALSE(is_modulable_v<std::add_pointer_t<Foo>>);
    }
  }
  ...
  GIVEN("a type with an user-defined operator+")
  {
    THEN("the trait value shall be true")
    {
      ...
      REQUIRE(is_modulable_v<Foo>);
      REQUIRE(is_modulable_v<Bar>);
    }
  }
}